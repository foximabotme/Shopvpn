...
def api_app_config(admin=Depends(get_current_admin)):
    ...
    if allowed("broadcast"):
        # بر خلاف کامنت قبلی، بک‌اند این تب هیچ انتخاب مخاطب/ضمیمه‌ای ندارد —
        # فقط متن ساده به همه‌ی کاربران (نگاه کن به BroadcastBody در همین
        # فایل) — برای همین کاملاً native شده.
        tabs.append({
